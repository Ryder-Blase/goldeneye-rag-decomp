...
- **Real decomp coverage (code bytes):** `100.000%`
  - Formula: source-owned code bytes / total code bytes from `splits.txt` code sections (`.init` + `.text`).
  - Current values:
    - `178,976 / 178,976` bytes (currently split across `src/system/main_before.cpp`, `src/gamemain.cpp`, `src/rtc.cpp`, and `src/system/drivecheck.cpp`).
